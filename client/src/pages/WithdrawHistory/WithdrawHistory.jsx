...
import { useNavigate } from "react-router"; // ← for back button
...
const WithdrawHistory = () => {
  ...
  const {
    ...
    queryKey: ["withdraw-history", userId],
    queryFn: async () => {
      if (!userId) throw new Error("User ID not found");

      const { data } = await axios.get(`${API}/api/withdraw-requests/withdraw-history/${userId}`);
      return data;
    },
    enabled: !!userId,
    retry: 1,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center gap-4">
        ...
        <span className="text-2xl text-orange-200">
          {isBangla ? "লোড হচ্ছে..." : "Loading history..."}
        </span>
      </div>
    );
  }
          ...
          {withdrawals.length === 0 ? (
            <div className="bg-black/50 rounded-2xl p-12 text-center border border-red-800/40 shadow-inner">
              ...
              <h3 className="text-2xl md:text-3xl font-semibold text-orange-100 mb-4">
                {isBangla ? "কোনো উত্তোলন রেকর্ড নেই" : "No withdrawal records yet"}
              </h3>
              <p className="text-orange-300/70 text-lg max-w-md mx-auto">
                {isBangla
                  ? "আপনি এখনও কোনো উত্তোলনের অনুরোধ করেননি"
