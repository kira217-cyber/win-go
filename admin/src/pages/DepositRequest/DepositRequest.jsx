// src/components/admin/DepositRequest.jsx
import React, { useState } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import axios from "axios";
import { toast } from "react-toastify";

const API = `${import.meta.env.VITE_API_URL}/api/deposit-requests`;
...
const DepositRequest = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["deposit-requests", filters],
    queryFn: () =>
      axios.get(API, { params: filters }).then((res) => res.data),
  });

  // Approve mutation (adds amount + bonus to user balance)
  const approveMutation = useMutation({
    mutationFn: ({ id, note }) => axios.put(`${API}/${id}/approve`, { note }),
    onSuccess: () => {
      toast.success("Deposit approved & added to user balance!");
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }) => axios.put(`${API}/${id}/reject`, { reason }),
    onSuccess: () => {
  ...
  if (isLoading) return <div className="text-center py-10 text-xl">Loading deposit requests...</div>;
  if (isError) return <div className="text-center text-red-600 py-10 text-xl">Failed to load requests</div>;
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <h1 className="text-2xl md:text-3xl font-bold mb-6 text-gray-800">Deposit Requests</h1>
          ...
          <div key={req._id} className=" rounded-xl shadow-md p-5">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
              ...
              <div className="col-span-2">
                ...
                <p>{req.method?.methodName?.en || "N/A"}</p>
              </div>
            </div>
      ...
      {approveModal.open && (
        <div className="fixed inset-0 bg-black/30 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md mx-4">
            <h3 className="text-xl font-bold mb-4">Approve Deposit Request</h3>
            <p className="text-gray-600 mb-4">
              Are you sure you want to approve this deposit? Bonus will be added to user's balance.
            </p>
            <textarea
              value={approveModal.note}
              onChange={(e) => setApproveModal({ ...approveModal, note: e.target.value })}
              placeholder="Optional approval note..."
              className="w-full border rounded-lg px-4 py-2 mb-4 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className=" rounded-xl p-6 w-full max-w-md mx-4">
            ...
            <textarea
              ...
              className="w-full border rounded-lg px-4 py-2 mb-4 min-h-[100px] focus:outline-none focus:ring-2 focus:ring-red-500"
              required
            />
            <div className="flex justify-end gap-4">
              <button
